  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-ink-900">Kelola Pengurus</h1>
          <p className="text-sm text-ink-600">Tambah, edit, hapus, dan atur urutan</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border bg-white p-8 text-center text-sm text-ink-600">Belum ada data pengurus.</div>
      ) : (
        <>
          {/* Desktop table */}
                ...
                {items.map((it, idx) => (
                  ...
                  >
                    <td
                      ...
                      aria-label={`Seret untuk mengubah urutan ${it.nama}`}
                    ...
                    <td className="px-4 py-3">
                      {it.foto ? <SmartImage src={it.foto} alt={it.nama} className="h-10 w-10 rounded object-cover border" /> : <span className="text-xs text-neutral-400">-</span>}
                    </td>
                    <td className="px-4 py-3 font-medium text-ink-900">{it.nama}</td>
                    <td className="px-4 py-3 text-ink-700">{it.jabatan}</td>
                    <td className="px-4 py-3 text-ink-600">{it.periode || '-'}</td>
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => move(idx, -1)} disabled={idx === 0} aria-label={`Naikkan urutan ${it.nama}`} className="rounded border px-2 py-1 text-xs disabled:opacity-40 focus-ring-light">↑</button>
                        <button onClick={() => move(idx, 1)} disabled={idx === items.length - 1} aria-label={`Turunkan urutan ${it.nama}`} className="rounded border px-2 py-1 text-xs disabled:opacity-40 focus-ring-light">↓</button>
            ...
            {items.map((it, idx) => (
              ...
              >
                ...
                <div className="flex gap-3">
                  {it.foto ? <SmartImage src={it.foto} alt={it.nama} className="h-14 w-14 rounded-lg object-cover border" /> : <div className="h-14 w-14 rounded-lg bg-neutral-100 flex items-center justify-center text-xs text-neutral-400">No Foto</div>}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-ink-900 truncate">{it.nama}</p>
                    <p className="text-xs text-ink-600">{it.jabatan} {it.periode ? `• ${it.periode}` : ''}</p>
                  </div>
                </div>
                <div className="mt-3 flex gap-2">
                  <button onClick={() => move(idx, -1)} disabled={idx === 0} aria-label={`Naikkan urutan ${it.nama}`} className="flex-1 rounded-lg border py-2 text-xs font-medium disabled:opacity-40 focus-ring-light">Naik</button>
                  <button onClick={() => move(idx, 1)} disabled={idx === items.length - 1} aria-label={`Turunkan urutan ${it.nama}`} className="flex-1 rounded-lg border py-2 text-xs font-medium disabled:opacity-40 focus-ring-light">Turun</button>
...
function PengurusForm({ initial, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    fd.append('jabatan', jabatan.trim())
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-auto rounded-t-2xl md:rounded-xl bg-white">
        <div className="sticky top-0 bg-white border-b px-5 py-4 flex items-center justify-between">
          <h2 className="text-base font-bold text-ink-900">{initial ? 'Edit Pengurus' : 'Tambah Pengurus'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-ink-700 mb-1">Nama *</label>
            <input value={nama} onChange={(e) => setNama(e.target.value)} className="w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm focus:border-gold-500 focus:ring-2 focus:ring-gold-500/20 outline-none" placeholder="Nama lengkap" />
            {fieldErrors.nama && <p className="text-xs text-danger-600 mt-1">{fieldErrors.nama}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-ink-700 mb-1">Jabatan *</label>
            <input value={jabatan} onChange={(e) => setJabatan(e.target.value)} className="w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm focus:border-gold-500 focus:ring-2 focus:ring-gold-500/20 outline-none" placeholder="Ketua, Wakil, dll" />
            {fieldErrors.jabatan && <p className="text-xs text-danger-600 mt-1">{fieldErrors.jabatan}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-ink-700 mb-1">Periode</label>
            <input value={periode} onChange={(e) => setPeriode(e.target.value)} className="w-full rounded-lg border border-neutral-300 px-3 py-2.5 text-sm focus:border-gold-500 focus:ring-2 focus:ring-gold-500/20 outline-none" placeholder="2024/2025" />
          </div>
          <div>
